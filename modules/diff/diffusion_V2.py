...
class AfterDiffusion(nn.Module):
    ...
    def forward(self, x):
        x = x.squeeze(1).permute(0, 2, 1).cuda()
...
class Pred(nn.Module):
    ...
    def forward(self, x_1, noise_t, t_1, t_prev):
        a_t = extract(self.alphas_cumprod, t_1).cuda()
        a_prev = extract(self.alphas_cumprod, t_prev).cuda()
        a_t_sq, a_prev_sq = a_t.sqrt().cuda(), a_prev.sqrt().cuda()
        x_delta = (a_prev - a_t) * ((1 / (a_t_sq * (a_t_sq + a_prev_sq))) * x_1 - 1 / (
                a_t_sq * (((1 - a_prev) * a_t).sqrt() + ((1 - a_t) * a_prev).sqrt())) * noise_t)
        x_pred = x_1 + x_delta.cuda()
        return x_pred
...
class GaussianDiffusion(nn.Module):
    ...
    def forward(self, hubert, mel2ph=None, spk_embed=None, f0=None, initial_noise=None, speedup=None, Onnx=False,
                project_name=None):
        ...
        for t in step_range:
            ...
            if plms_noise_stage == 0:
                if Onnx:
                    torch.onnx.export(
                        self.xp,
                        (x.cuda(), noise_pred.cuda(), t_1.cuda(), t_prev.cuda()),
